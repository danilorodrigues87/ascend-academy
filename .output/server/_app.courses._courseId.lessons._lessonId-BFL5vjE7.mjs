import { r as __toESM } from "./_runtime.mjs";
import { u as require_react } from "./_libs/@floating-ui/react-dom+[...].mjs";
import { p as require_jsx_runtime } from "./_libs/@radix-ui/react-avatar+[...].mjs";
import { t as Button } from "./_ssr/button-Bq5vK6RO.mjs";
import { t as Skeleton } from "./_ssr/skeleton-D9W9wFsj.mjs";
import { n as formatMinutes } from "./_ssr/format-BkgF6Xya.mjs";
import { t as Input } from "./_ssr/input-B8Q2ztVi.mjs";
import { _ as useNavigate, g as Link } from "./_libs/@tanstack/react-router+[...].mjs";
import { i as useQueryClient, n as useQuery, t as useMutation } from "./_libs/tanstack__react-query.mjs";
import { A as Lock, E as MessageCircleMore, G as ClipboardCheck, H as ExternalLink, J as CirclePlay, M as Link$1, R as FileText, U as Download, Y as CircleCheck, at as ArrowLeft, b as PanelRightClose, f as Square, h as Send, it as ArrowRight, p as Sparkles, y as PanelRightOpen } from "./_libs/lucide-react.mjs";
import { t as aiService } from "./_ssr/aiService-Dyh38p1f.mjs";
import { t as Textarea } from "./_ssr/textarea-kko37XEX.mjs";
import { t as assessmentsService } from "./_ssr/assessmentsService-BIlrrkBP.mjs";
import { t as Card } from "./_ssr/card-CzXpCsbD.mjs";
import { n as toast } from "./_libs/sonner.mjs";
import { t as coursesService } from "./_ssr/coursesService-DXN4SuWd.mjs";
import { t as rolePlayService } from "./_ssr/rolePlayService-Dus-U4cn.mjs";
import { t as Route } from "./_app.courses._courseId.lessons._lessonId-ykLDh9tn.mjs";
import { i as TabsTrigger, n as TabsContent, r as TabsList, t as Tabs } from "./_ssr/tabs-CCJRliUM.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/_app.courses._courseId.lessons._lessonId-BFL5vjE7.js
var import_react = /* @__PURE__ */ __toESM(require_react());
var import_jsx_runtime = require_jsx_runtime();
/** Extrai ID e monta URL embed do YouTube. */
function youtubeVideoId(url) {
	const u = (url || "").trim();
	if (!u) return null;
	if (/^[a-zA-Z0-9_-]{11}$/.test(u)) return u;
	return u.match(/(?:youtube\.com\/(?:watch\?(?:.*&)?v=|embed\/|shorts\/|live\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/)?.[1] ?? null;
}
function toYoutubeEmbed(url) {
	const id = youtubeVideoId(url);
	return id ? `https://www.youtube.com/embed/${id}` : null;
}
function youtubeEmbedSrc(url, params = "rel=0&modestbranding=1") {
	const embed = toYoutubeEmbed(url);
	if (!embed) return null;
	return embed.includes("?") ? `${embed}&${params}` : `${embed}?${params}`;
}
function InlineAssessment({ assessmentId, onBack }) {
	const qc = useQueryClient();
	const { data, isLoading, refetch } = useQuery({
		queryKey: ["assessment", assessmentId],
		queryFn: () => assessmentsService.getById(assessmentId)
	});
	const [qIndex, setQIndex] = (0, import_react.useState)(0);
	const [draft, setDraft] = (0, import_react.useState)("");
	const [result, setResult] = (0, import_react.useState)(null);
	const [busy, setBusy] = (0, import_react.useState)(false);
	(0, import_react.useEffect)(() => {
		setQIndex(0);
		setDraft("");
		setResult(null);
	}, [assessmentId]);
	const startMut = useMutation({
		mutationFn: () => assessmentsService.start(assessmentId),
		onSuccess: () => refetch(),
		onError: (e) => toast.error(e.message || "Não foi possível iniciar.")
	});
	if (isLoading) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Skeleton, { className: "h-64 w-full rounded-xl" });
	if (!data) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
		className: "text-muted-foreground",
		children: "Atividade não encontrada."
	});
	const attempt = data.attempt;
	const lockedMap = attempt?.answers ?? {};
	const questions = data.questions;
	const q = questions[qIndex];
	const locked = q ? lockedMap[q.id] : void 0;
	const allLocked = questions.length > 0 && questions.every((qq) => lockedMap[qq.id]?.locked);
	const inProgress = attempt?.status === "in_progress" || attempt?.canAnswer;
	const finished = attempt?.status === "completed" && attempt.score != null;
	const confirmAnswer = async () => {
		if (!q || !draft.trim() || busy || locked) return;
		setBusy(true);
		try {
			if (!inProgress) await assessmentsService.start(assessmentId);
			const r = await assessmentsService.answer(assessmentId, q.id, draft.trim());
			setDraft("");
			await refetch();
			if (r.correct === true) toast.success(`Correto · ${r.score}%`);
			else if (r.correct === false) toast.message(`Registrado · ${r.score}%`);
			else toast.success("Resposta registrada");
			if (r.allAnswered) {
				const fin = await assessmentsService.finalize(assessmentId);
				setResult(fin);
				toast.success(`Atividade finalizada: ${fin.score}%${fin.xpEarned ? ` · +${fin.xpEarned} XP` : ""}`);
				qc.invalidateQueries({ queryKey: ["course"] });
				qc.invalidateQueries({ queryKey: ["ranking"] });
				await refetch();
			} else if (qIndex < questions.length - 1) setQIndex((i) => i + 1);
		} catch (e) {
			toast.error(e instanceof Error ? e.message : "Falha ao registrar resposta");
		} finally {
			setBusy(false);
		}
	};
	if (result || finished && !inProgress && allLocked) {
		const score = result?.score ?? attempt?.score ?? 0;
		const feedback = result?.feedback ?? attempt?.feedback ?? "";
		const xp = result?.xpEarned;
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Card, {
			className: "space-y-4 p-6",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "flex items-start justify-between gap-4",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "text-xs uppercase tracking-wider text-muted-foreground",
						children: "Resultado"
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", {
						className: "font-display text-2xl font-semibold",
						children: data.title
					})] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
						variant: "outline",
						onClick: onBack,
						children: "Voltar à aula"
					})]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "font-display text-4xl font-semibold",
					children: [score, "%"]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "text-sm text-muted-foreground",
					children: feedback
				}),
				xp != null && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "text-sm font-medium text-primary",
					children: [
						"+",
						xp,
						" XP"
					]
				}),
				result?.needsRewatch && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "text-sm text-amber-600",
					children: "Média da aula abaixo de 70%. Assista a aula novamente para liberar +3 tentativas."
				}),
				result?.unitScore != null && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "text-sm text-muted-foreground",
					children: [
						"Média da unidade: ",
						result.unitScore,
						"%"
					]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "text-xs text-muted-foreground",
					children: [
						"Tentativas: ",
						attempt?.attemptsUsed ?? 0,
						"/",
						attempt?.attemptsMax ?? data.attempts
					]
				}),
				attempt?.canStart && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
					onClick: () => {
						setResult(null);
						setQIndex(0);
						startMut.mutate();
					},
					children: "Nova tentativa"
				})
			]
		});
	}
	if (!inProgress && !attempt?.canAnswer) return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Card, {
		className: "space-y-4 p-6",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "flex items-start justify-between gap-4",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "text-xs uppercase tracking-wider text-muted-foreground",
					children: "Atividade"
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", {
					className: "font-display text-2xl font-semibold",
					children: data.title
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "mt-1 text-sm text-muted-foreground",
					children: data.description
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "text-xs text-muted-foreground",
					children: [
						questions.length,
						" questões · ",
						data.attempt?.attemptsMax ?? 3,
						" tentativas por ciclo · respostas definitivas"
					]
				}),
				data.attempt?.rewatchHint && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "mt-2 text-sm text-amber-600",
					children: data.attempt.rewatchHint
				}),
				data.unitScore != null && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "mt-1 text-xs text-muted-foreground",
					children: [
						"Média da unidade (atividades + roleplay): ",
						data.unitScore,
						"%"
					]
				})
			] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
				variant: "outline",
				onClick: onBack,
				children: "Voltar à aula"
			})]
		}), attempt?.canStart === false ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
			className: "text-sm text-destructive",
			children: "Limite de tentativas esgotado."
		}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
			onClick: () => startMut.mutate(),
			disabled: startMut.isPending,
			children: "Começar atividade"
		})]
	});
	if (!q) return null;
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Card, {
		className: "space-y-4 p-6",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "flex items-start justify-between gap-4",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "text-xs uppercase tracking-wider text-muted-foreground",
					children: [
						"Questão ",
						qIndex + 1,
						" de ",
						questions.length
					]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", {
					className: "font-display text-2xl font-semibold",
					children: data.title
				})] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
					variant: "outline",
					onClick: onBack,
					children: "Voltar à aula"
				})]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "rounded-xl border border-border/60 p-4",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "text-sm font-medium",
					children: q.prompt
				}), locked ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "mt-3 space-y-2 text-sm",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { children: [
							"Sua resposta: ",
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: locked.answer }),
							locked.correct === true && " — Correto",
							locked.correct === false && " — Incorreto"
						] }),
						locked.feedback && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "text-muted-foreground",
							children: locked.feedback
						}),
						locked.score != null && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
							className: "text-muted-foreground",
							children: [
								"Nota desta questão: ",
								locked.score,
								"%"
							]
						}),
						qIndex < questions.length - 1 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
							onClick: () => {
								setQIndex((i) => i + 1);
								setDraft("");
							},
							children: "Próxima questão"
						}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "text-xs text-muted-foreground",
							children: "Todas respondidas — finalizando…"
						})
					]
				}) : q.type === "essay" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Textarea, {
					className: "mt-3",
					value: draft,
					onChange: (e) => setDraft(e.target.value),
					placeholder: "Digite sua resposta (será corrigida por IA)",
					disabled: busy
				}) : q.type === "boolean" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "mt-3 flex flex-wrap gap-2",
					children: (q.options?.length ? q.options : [{
						id: "true",
						label: "Verdadeiro"
					}, {
						id: "false",
						label: "Falso"
					}]).map((opt) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
						type: "button",
						variant: draft === opt.id ? "default" : "outline",
						onClick: () => setDraft(opt.id),
						disabled: busy,
						children: opt.label
					}, opt.id))
				}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "mt-3 space-y-2",
					children: (q.options ?? []).map((opt) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
						className: "flex cursor-pointer items-center gap-2 rounded-lg border border-border/50 px-3 py-2 text-sm hover:bg-accent/30",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
							type: "radio",
							name: q.id,
							checked: draft === opt.id,
							onChange: () => setDraft(opt.id),
							disabled: busy
						}), opt.label]
					}, opt.id))
				})]
			}),
			!locked && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
				disabled: busy || !draft.trim(),
				onClick: () => void confirmAnswer(),
				children: busy ? "Corrigindo…" : "Confirmar resposta"
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
				className: "text-xs text-muted-foreground",
				children: "Após confirmar, a resposta não pode ser alterada."
			})
		]
	});
}
function InlineRoleplay({ scenarioId, courseId, onDone }) {
	const { data: scenario, isLoading } = useQuery({
		queryKey: ["roleplay:scenario", scenarioId],
		queryFn: () => rolePlayService.getScenario(scenarioId)
	});
	const [sim, setSim] = (0, import_react.useState)(null);
	const [input, setInput] = (0, import_react.useState)("");
	const [busy, setBusy] = (0, import_react.useState)(false);
	const [remaining, setRemaining] = (0, import_react.useState)(null);
	(0, import_react.useEffect)(() => {
		if (remaining == null || remaining <= 0 || !sim || sim.endedAt) return;
		const t = setInterval(() => setRemaining((r) => r == null ? r : Math.max(0, r - 1)), 1e3);
		return () => clearInterval(t);
	}, [remaining, sim]);
	(0, import_react.useEffect)(() => {
		if (remaining === 0 && sim && !sim.endedAt) finishNow();
	}, [remaining]);
	const start = async () => {
		setBusy(true);
		try {
			const s = await rolePlayService.startSimulation(scenarioId, "medium");
			setSim(s);
			setRemaining(s.timeRemainingSeconds ?? (scenario?.estimatedMinutes ?? 15) * 60);
		} catch {
			toast.error("Não foi possível iniciar o role play.");
		} finally {
			setBusy(false);
		}
	};
	const send = async () => {
		if (!sim || !input.trim() || busy) return;
		const content = input.trim();
		setInput("");
		setBusy(true);
		setSim((prev) => prev ? {
			...prev,
			messages: [...prev.messages, {
				id: `u_${Date.now()}`,
				role: "user",
				content,
				createdAt: (/* @__PURE__ */ new Date()).toISOString()
			}]
		} : prev);
		try {
			const aiMsg = await rolePlayService.sendMessage(sim.id, content);
			setSim((prev) => prev ? {
				...prev,
				messages: [...prev.messages, aiMsg]
			} : prev);
		} catch (e) {
			toast.error(e instanceof Error ? e.message : "Falha ao enviar");
		} finally {
			setBusy(false);
		}
	};
	const finishNow = async () => {
		if (!sim || busy) return;
		setBusy(true);
		try {
			const done = await rolePlayService.finishSimulation(sim.id);
			setSim(done);
			setRemaining(0);
			toast.success(`Role play finalizado: ${done.score ?? 0}%`);
			onDone();
		} catch {
			toast.error("Não foi possível finalizar.");
		} finally {
			setBusy(false);
		}
	};
	if (isLoading) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Skeleton, { className: "h-64 w-full rounded-xl" });
	if (!scenario) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
		className: "text-muted-foreground",
		children: "Cenário não encontrado."
	});
	if (!sim) return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Card, {
		className: "space-y-4 p-8 text-center",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)(MessageCircleMore, { className: "mx-auto h-10 w-10 text-primary" }),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", {
				className: "font-display text-xl font-semibold",
				children: scenario.title
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
				className: "text-sm text-muted-foreground",
				children: scenario.scenario || scenario.theme
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
				className: "text-xs text-muted-foreground",
				children: [
					"Tempo limite: ",
					scenario.estimatedMinutes,
					" min · Nota mínima: ",
					scenario.minScore,
					"%"
				]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
				className: "text-xs text-muted-foreground",
				children: ["Curso ", courseId]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
				onClick: () => void start(),
				disabled: busy,
				children: "Iniciar simulação"
			})
		]
	});
	const mm = remaining != null ? Math.floor(remaining / 60) : 0;
	const ss = remaining != null ? remaining % 60 : 0;
	if (!!sim.endedAt || sim.status === "approved" || sim.status === "retry") return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Card, {
		className: "space-y-4 p-6",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("h2", {
				className: "font-display text-xl font-semibold",
				children: ["Resultado — ", scenario.title]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
				className: "font-display text-4xl font-semibold",
				children: [sim.score ?? 0, "%"]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
				className: "text-sm text-muted-foreground",
				children: sim.evaluation?.summary
			}),
			sim.evaluation?.strengths?.length ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
				className: "text-xs font-medium uppercase text-muted-foreground",
				children: "Pontos fortes"
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("ul", {
				className: "mt-1 list-disc pl-5 text-sm",
				children: sim.evaluation.strengths.map((s) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("li", { children: s }, s))
			})] }) : null
		]
	});
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Card, {
		className: "flex min-h-[420px] flex-col p-0",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "flex items-center justify-between border-b border-border/60 px-4 py-3",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "font-display text-lg font-semibold",
					children: scenario.title
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "text-xs text-muted-foreground",
					children: [
						"Você: ",
						scenario.userRole,
						" · Personagem: ",
						scenario.aiCharacterName
					]
				})] }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "text-right text-sm",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
						className: remaining != null && remaining < 60 ? "font-semibold text-destructive" : "font-medium",
						children: [
							String(mm).padStart(2, "0"),
							":",
							String(ss).padStart(2, "0")
						]
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
						size: "sm",
						variant: "outline",
						onClick: () => void finishNow(),
						disabled: busy,
						children: "Finalizar"
					})]
				})]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "flex-1 space-y-3 overflow-y-auto p-4",
				children: sim.messages.map((m) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: `rounded-xl px-3 py-2 text-sm ${m.role === "user" ? "ml-8 bg-primary/10" : "mr-8 bg-muted"}`,
					children: m.content
				}, m.id))
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("form", {
				className: "flex gap-2 border-t border-border/60 p-3",
				onSubmit: (e) => {
					e.preventDefault();
					send();
				},
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Input, {
					value: input,
					onChange: (e) => setInput(e.target.value),
					placeholder: "Sua fala na simulação…",
					disabled: busy
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
					type: "submit",
					size: "icon",
					disabled: busy || !input.trim(),
					children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Send, { className: "h-4 w-4" })
				})]
			})
		]
	});
}
function LessonPage() {
	const { courseId, lessonId } = Route.useParams();
	const search = Route.useSearch();
	const navigate = useNavigate();
	const qc = useQueryClient();
	const [sidebarOpen, setSidebarOpen] = (0, import_react.useState)(true);
	const [sideTab, setSideTab] = (0, import_react.useState)(search.panel === "ai" ? "ai" : "content");
	const { data, isLoading, isError } = useQuery({
		queryKey: [
			"lesson",
			courseId,
			lessonId
		],
		queryFn: () => coursesService.getLesson(courseId, lessonId)
	});
	const [notes, setNotes] = (0, import_react.useState)("");
	(0, import_react.useEffect)(() => {
		const saved = localStorage.getItem(`notes:${lessonId}`);
		setNotes(saved ?? "");
	}, [lessonId]);
	(0, import_react.useEffect)(() => {
		const t = setTimeout(() => localStorage.setItem(`notes:${lessonId}`, notes), 400);
		return () => clearTimeout(t);
	}, [notes, lessonId]);
	const complete = useMutation({
		mutationFn: () => coursesService.markLessonCompleted(courseId, lessonId),
		onSuccess: (res) => {
			toast.success(res?.message || "Aula marcada como concluída!");
			qc.invalidateQueries({ queryKey: [
				"lesson",
				courseId,
				lessonId
			] });
			qc.invalidateQueries({ queryKey: ["course", courseId] });
			qc.invalidateQueries({ queryKey: ["courses"] });
			qc.invalidateQueries({ queryKey: ["dashboard"] });
			qc.invalidateQueries({ queryKey: ["ranking"] });
		}
	});
	const activeItem = (0, import_react.useMemo)(() => {
		if (!search.item || !data) return {
			kind: "lesson",
			id: lessonId
		};
		const [kind, id] = search.item.split(":");
		if ((kind === "assessment" || kind === "roleplay") && id) return {
			kind,
			id
		};
		return {
			kind: "lesson",
			id: lessonId
		};
	}, [
		search.item,
		lessonId,
		data
	]);
	const { prev, next } = (0, import_react.useMemo)(() => {
		if (!data) return {
			prev: null,
			next: null
		};
		const flat = data.course.modules.flatMap((m) => m.curriculum ?? m.lessons.map((l) => ({
			kind: "lesson",
			id: l.id,
			title: l.title,
			order: l.order,
			durationMinutes: l.durationMinutes,
			completed: l.completed,
			locked: l.locked
		})));
		let idx = -1;
		if (activeItem.kind === "lesson") idx = flat.findIndex((i) => i.kind === "lesson" && i.id === data.lesson.id);
		else idx = flat.findIndex((i) => i.kind === activeItem.kind && i.id === activeItem.id);
		return {
			prev: flat[idx - 1] ?? null,
			next: flat[idx + 1] ?? null
		};
	}, [data, activeItem]);
	if (isLoading) return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "mx-auto max-w-7xl space-y-4 p-4 md:p-6",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Skeleton, { className: "aspect-video w-full rounded-2xl" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Skeleton, { className: "h-8 w-1/2" })]
	});
	if (isError || !data) return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "space-y-3 p-8",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
				className: "text-muted-foreground",
				children: "NÃ£o foi possÃ­vel carregar a aula."
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
				className: "text-xs text-muted-foreground",
				children: [
					"Curso ",
					courseId,
					" Â· Aula ",
					lessonId
				]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
				asChild: true,
				variant: "outline",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Link, {
					to: "/courses",
					children: "Voltar aos cursos"
				})
			})
		]
	});
	const { course, lesson } = data;
	const goItem = (item) => {
		if (item.locked) return;
		if (item.kind === "lesson") {
			navigate({
				to: "/courses/$courseId/lessons/$lessonId",
				params: {
					courseId: course.id,
					lessonId: item.id
				},
				search: { panel: sideTab }
			});
			return;
		}
		navigate({
			to: "/courses/$courseId/lessons/$lessonId",
			params: {
				courseId: course.id,
				lessonId
			},
			search: {
				panel: sideTab,
				item: `${item.kind}:${item.id}`
			}
		});
	};
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: `mx-auto grid max-w-[1600px] gap-0 p-0 md:p-2 ${sidebarOpen ? "lg:grid-cols-[minmax(0,1fr)_380px]" : "grid-cols-1"}`,
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "min-w-0 space-y-4 p-4 md:p-6",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "flex items-center justify-between gap-2",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Link, {
						to: "/courses",
						className: "inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ArrowLeft, { className: "h-4 w-4" }), " Meus cursos"]
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
						variant: "ghost",
						size: "sm",
						onClick: () => setSidebarOpen((v) => !v),
						className: "gap-1",
						children: [sidebarOpen ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(PanelRightClose, { className: "h-4 w-4" }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(PanelRightOpen, { className: "h-4 w-4" }), sidebarOpen ? "Ocultar" : "ConteÃºdo"]
					})]
				}),
				activeItem.kind === "lesson" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(VideoPlayer, { lesson }),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", {
						className: "font-display text-2xl font-semibold md:text-3xl",
						children: lesson.title
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
						className: "mt-1 text-sm text-muted-foreground",
						children: [
							formatMinutes(lesson.durationMinutes),
							" Â· ",
							course.title
						]
					})] }),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "flex flex-wrap items-center gap-2",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
							onClick: () => complete.mutate(),
							disabled: lesson.completed || complete.isPending,
							className: "gap-2",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(CircleCheck, { className: "h-4 w-4" }), lesson.completed ? "ConcluÃ­da" : "Marcar como concluÃ­da"]
						}), next && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
							variant: "outline",
							className: "gap-2",
							onClick: () => goItem(next),
							children: ["PrÃ³ximo ", /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ArrowRight, { className: "h-4 w-4" })]
						})]
					})
				] }),
				activeItem.kind === "assessment" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(InlineAssessment, {
					courseId: course.id,
					assessmentId: activeItem.id,
					onBack: () => navigate({
						to: "/courses/$courseId/lessons/$lessonId",
						params: {
							courseId: course.id,
							lessonId
						},
						search: { panel: sideTab }
					})
				}),
				activeItem.kind === "roleplay" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(InlineRoleplay, {
					scenarioId: activeItem.id,
					courseId: course.id,
					onDone: () => {
						qc.invalidateQueries({ queryKey: ["course", course.id] });
						qc.invalidateQueries({ queryKey: [
							"lesson",
							courseId,
							lessonId
						] });
						qc.invalidateQueries({ queryKey: ["ranking"] });
					}
				}),
				activeItem.kind === "lesson" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Tabs, {
					defaultValue: "about",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(TabsList, { children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(TabsTrigger, {
								value: "about",
								children: "VisÃ£o geral"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(TabsTrigger, {
								value: "resources",
								children: "Materiais"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(TabsTrigger, {
								value: "notes",
								children: "AnotaÃ§Ãµes"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(TabsTrigger, {
								value: "comments",
								children: "ComentÃ¡rios"
							})
						] }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(TabsContent, {
							value: "about",
							className: "mt-4",
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Card, {
								className: "space-y-3 p-6 text-sm leading-relaxed text-muted-foreground",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
									className: "font-display text-lg font-semibold text-foreground",
									children: course.title
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: lesson.description || course.shortDescription || "Sem descriÃ§Ã£o." })]
							})
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(TabsContent, {
							value: "resources",
							className: "mt-4",
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Card, {
								className: "divide-y divide-border/60",
								children: (lesson.resources ?? []).length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
									className: "p-6 text-sm text-muted-foreground",
									children: "Nenhum material nesta aula."
								}) : (lesson.resources ?? []).map((r) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("a", {
									href: r.url,
									target: "_blank",
									rel: "noreferrer",
									className: "flex items-center gap-3 p-4 transition hover:bg-accent/40",
									children: [
										r.type === "pdf" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(FileText, { className: "h-4 w-4 text-primary" }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Link$1, { className: "h-4 w-4 text-primary" }),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
											className: "flex-1 text-sm",
											children: r.label
										}),
										r.type === "pdf" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Download, { className: "h-4 w-4 text-muted-foreground" }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ExternalLink, { className: "h-4 w-4 text-muted-foreground" })
									]
								}, r.id))
							})
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(TabsContent, {
							value: "notes",
							className: "mt-4",
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Card, {
								className: "p-4",
								children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Textarea, {
									placeholder: "Suas anotaÃ§Ãµes desta aula (salvamento automÃ¡tico)...",
									value: notes,
									onChange: (e) => setNotes(e.target.value),
									className: "min-h-[180px] border-0 focus-visible:ring-0"
								})
							})
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(TabsContent, {
							value: "comments",
							className: "mt-4",
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Card, {
								className: "p-8 text-center text-sm text-muted-foreground",
								children: "Em breve: comentÃ¡rios entre alunos e resposta do professor."
							})
						})
					]
				})
			]
		}), sidebarOpen && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("aside", {
			className: "min-w-0 border-l border-border/60 bg-card/40",
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "sticky top-0 flex h-[calc(100vh-1rem)] flex-col",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "flex border-b border-border/60",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
						type: "button",
						onClick: () => setSideTab("content"),
						className: `flex-1 px-3 py-3 text-sm font-medium ${sideTab === "content" ? "border-b-2 border-primary text-foreground" : "text-muted-foreground"}`,
						children: "ConteÃºdo do curso"
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
						type: "button",
						onClick: () => setSideTab("ai"),
						className: `flex flex-1 items-center justify-center gap-1 px-3 py-3 text-sm font-medium ${sideTab === "ai" ? "border-b-2 border-primary text-foreground" : "text-muted-foreground"}`,
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Sparkles, { className: "h-3.5 w-3.5" }), " Assistente IA"]
					})]
				}), sideTab === "content" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CourseCurriculum, {
					course,
					activeLessonId: lesson.id,
					activeItem,
					onSelect: goItem
				}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CourseAiAssistant, {
					course,
					lesson
				})]
			})
		})]
	});
}
function CourseCurriculum({ course, activeLessonId, activeItem, onSelect }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "flex-1 overflow-y-auto",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "border-b border-border/60 p-4",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "text-xs uppercase tracking-wider text-muted-foreground",
					children: "Curso"
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "mt-1 truncate font-medium",
					children: course.title
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "mt-1 text-xs text-muted-foreground",
					children: [course.progressPercent, "% concluÃ­do"]
				})
			]
		}), course.modules.map((m, mi) => {
			const items = m.curriculum ?? m.lessons.map((l) => ({
				kind: "lesson",
				id: l.id,
				title: l.title,
				order: l.order,
				durationMinutes: l.durationMinutes,
				completed: l.completed,
				locked: l.locked
			}));
			const done = items.filter((i) => i.completed).length;
			const mins = items.reduce((s, i) => s + (i.durationMinutes ?? 0), 0);
			const roleplays = items.filter((i) => i.kind === "roleplay").length;
			const assessments = items.filter((i) => i.kind === "assessment").length;
			return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "border-b border-border/40 bg-muted/30 px-4 py-2.5",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "text-xs font-semibold uppercase tracking-wider text-muted-foreground",
					children: [
						"SeÃ§Ã£o ",
						mi + 1,
						" â€” ",
						m.title
					]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "mt-1 text-[11px] text-muted-foreground",
					children: [
						done,
						" / ",
						items.length,
						" Â· ",
						mins,
						"m",
						assessments > 0 ? ` Â· ${assessments} atividade(s)` : "",
						roleplays > 0 ? ` Â· ${roleplays} role play` : ""
					]
				})]
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("ul", { children: items.map((it) => {
				const active = it.kind === "lesson" && it.id === activeLessonId && activeItem.kind === "lesson" || it.kind === activeItem.kind && it.id === activeItem.id;
				return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("li", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
					type: "button",
					disabled: it.locked,
					onClick: () => onSelect(it),
					className: `flex w-full items-center gap-2 border-b border-border/30 px-4 py-2.5 text-left text-sm transition-colors ${active ? "bg-primary/10 text-primary" : "hover:bg-accent/40"} ${it.locked ? "opacity-60" : ""}`,
					children: [
						it.locked ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Lock, { className: "h-3.5 w-3.5 shrink-0" }) : it.completed ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CircleCheck, { className: "h-3.5 w-3.5 shrink-0 text-success" }) : it.kind === "assessment" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ClipboardCheck, { className: "h-3.5 w-3.5 shrink-0 text-amber-600" }) : it.kind === "roleplay" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(MessageCircleMore, { className: "h-3.5 w-3.5 shrink-0 text-emerald-600" }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Square, { className: "h-3.5 w-3.5 shrink-0" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "flex-1 truncate",
							children: it.title
						}),
						it.kind !== "lesson" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "shrink-0 rounded bg-primary/10 px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-primary",
							children: it.kind === "assessment" ? "Atividade" : "Role play"
						}),
						it.durationMinutes != null && it.durationMinutes > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
							className: "shrink-0 text-xs text-muted-foreground",
							children: [it.durationMinutes, "m"]
						})
					]
				}) }, `${it.kind}_${it.id}`);
			}) })] }, m.id);
		})]
	});
}
function VideoPlayer({ lesson }) {
	const videos = lesson.videos?.length ? lesson.videos : lesson.videoUrl ? [{
		id: "0",
		title: lesson.title,
		url: lesson.videoUrl,
		provider: lesson.videoProvider || "youtube",
		durationMinutes: lesson.durationMinutes,
		order: 0
	}] : [];
	const [idx, setIdx] = (0, import_react.useState)(0);
	const current = videos[idx];
	if (!current) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		className: "flex aspect-video items-center justify-center rounded-2xl border border-dashed border-border/60 bg-muted/30 text-sm text-muted-foreground",
		children: "Esta aula nÃ£o tem vÃ­deo â€” confira a descriÃ§Ã£o e os materiais abaixo."
	});
	const provider = current.provider || "youtube";
	const embed = provider === "youtube" ? youtubeEmbedSrc(current.url) : null;
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "space-y-2",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			className: "relative aspect-video overflow-hidden rounded-2xl border border-border/60 bg-black shadow-elegant",
			children: provider === "youtube" && embed ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("iframe", {
				src: embed,
				title: current.title,
				allow: "accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture",
				allowFullScreen: true,
				className: "h-full w-full"
			}) : provider === "private" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("video", {
				src: current.url,
				controls: true,
				className: "h-full w-full"
			}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "grid h-full place-items-center p-6 text-center text-sm text-white/80",
				children: "NÃ£o foi possÃ­vel carregar este vÃ­deo. Verifique o link no painel (cole o URL completo do YouTube)."
			})
		}), videos.length > 1 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			className: "flex flex-wrap gap-2",
			children: videos.map((v, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
				size: "sm",
				variant: i === idx ? "default" : "outline",
				onClick: () => setIdx(i),
				className: "gap-1",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(CirclePlay, { className: "h-3.5 w-3.5" }),
					" ",
					v.title || `VÃ­deo ${i + 1}`
				]
			}, v.id))
		})]
	});
}
function CourseAiAssistant({ course, lesson }) {
	const [convId, setConvId] = (0, import_react.useState)(null);
	const [messages, setMessages] = (0, import_react.useState)([]);
	const [input, setInput] = (0, import_react.useState)("");
	const [busy, setBusy] = (0, import_react.useState)(false);
	const suggestions = [
		`Resuma a aula "${lesson.title}"`,
		`Quais pontos principais de ${course.title}?`,
		"Explique o conteÃºdo de forma simples",
		"O que revisar antes da prÃ³xima atividade?"
	];
	const ensureConv = async () => {
		if (convId) return convId;
		const c = await aiService.createConversation(`${course.title} â€” ${lesson.title}`, {
			courseId: course.id,
			lessonId: lesson.id
		});
		setConvId(c.id);
		return c.id;
	};
	const send = async (text) => {
		const content = text.trim();
		if (!content || busy) return;
		setBusy(true);
		setMessages((m) => [...m, {
			role: "user",
			content
		}]);
		setInput("");
		try {
			const id = await ensureConv();
			const reply = await aiService.sendMessage(id, content, {
				courseId: course.id,
				lessonId: lesson.id
			});
			setMessages((m) => [...m, {
				role: "assistant",
				content: reply.content
			}]);
		} catch {
			toast.error("Assistente indisponÃ­vel no momento.");
			setMessages((m) => m.slice(0, -1));
		} finally {
			setBusy(false);
		}
	};
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "flex flex-1 flex-col overflow-hidden",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "space-y-2 border-b border-border/60 p-4",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h3", {
					className: "font-display text-lg font-semibold",
					children: "DÃºvidas sobre este curso?"
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "text-xs text-muted-foreground",
					children: "O assistente pode cometer erros. Confira as informaÃ§Ãµes. Sujeito Ã\xA0 configuraÃ§Ã£o de IA da escola."
				})]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "flex-1 space-y-3 overflow-y-auto p-4",
				children: [messages.length === 0 && suggestions.map((s) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
					type: "button",
					onClick: () => send(s),
					className: "block w-full rounded-xl border border-border/60 bg-background px-3 py-3 text-left text-sm hover:bg-accent/40",
					children: s
				}, s)), messages.map((m, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: `rounded-xl px-3 py-2 text-sm ${m.role === "user" ? "ml-6 bg-primary/10" : "mr-6 bg-muted"}`,
					children: m.content
				}, i))]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "border-t border-border/60 p-3",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("form", {
					className: "flex gap-2",
					onSubmit: (e) => {
						e.preventDefault();
						send(input);
					},
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Input, {
						value: input,
						onChange: (e) => setInput(e.target.value),
						placeholder: "Fazer uma pergunta",
						disabled: busy
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
						type: "submit",
						size: "icon",
						disabled: busy || !input.trim(),
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Send, { className: "h-4 w-4" })
					})]
				})
			})
		]
	});
}
//#endregion
export { LessonPage as component };
